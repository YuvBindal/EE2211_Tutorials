...
def main():
    ...
    try:
        ...
        if inp =='1':
            text = "Linear Regression - remember to add bias BUT SYSTEMS OF LINEAR EQUATIONS got NO biased term"
            print(f"{Fore.GREEN}{Style.BRIGHT}{text}{Style.RESET_ALL}")    
            linear_regression("non-ridge", "regression")
            
        if inp =='11':
            text = "Linear Regression with Ridge Regression - remember to add bias"
            print(f"{Fore.GREEN}{Style.BRIGHT}{text}{Style.RESET_ALL}")    
            linear_regression("ridge", "regression")
        
        if inp =='2':
            text = "Linear Classification - remember to add bias"
            print(f"{Fore.GREEN}{Style.BRIGHT}{text}{Style.RESET_ALL}")    
            linear_regression("non-ridge", "classification")
        
        if inp =='21':
            text = "Linear Classification with Ridge Regression - remember to add bias"
            print(f"{Fore.GREEN}{Style.BRIGHT}{text}{Style.RESET_ALL}")    
            linear_regression("ridge", "classification")
        
        if inp =='3':
            text = "Polynomial Regression - auto bias"
            print(f"{Fore.GREEN}{Style.BRIGHT}{text}{Style.RESET_ALL}")    
            polynomial_regression("non-ridge", "regression")
            
        if inp =='31':
            text = "Polynomial Regression with Ridge Regression - auto bias"
            print(f"{Fore.GREEN}{Style.BRIGHT}{text}{Style.RESET_ALL}")    
            polynomial_regression("ridge", "regression")
        
        if inp =='4':
            text = "Polynomial Classification - auto bias"
            print(f"{Fore.GREEN}{Style.BRIGHT}{text}{Style.RESET_ALL}")    
            polynomial_regression("non-ridge", "classification")
        
        if inp =='41':
            text = "Polynomial Classification with Ridge Regression - auto bias"
            print(f"{Fore.GREEN}{Style.BRIGHT}{text}{Style.RESET_ALL}")    
...
def k_classification():
    ...
    print(X)
    print(X.shape)
    print("Please input the initialised centers (first row is x values, second row is y values)")
...
def misclassification_tree():
    ...
    print("Please input Leaf Distribution (Exclude Root Matrix here)")
...
def gradient_descent():
    ...
    eqn = lambda x : 4*x**3  # REMINDER TO CHANGE THIS EQUATION, lambda x : (EQN HERE MUST BE DIFFERENTIATED) e.g. math.cos(x) or 4*x**3 (4x^3)
    x_init = float(input("Initial x value:\n"))
